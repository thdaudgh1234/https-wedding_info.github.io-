class Example extends Phaser.Scene {
    ...
    startGame() {
		...
		this.anims.create({ key: 'cat_wake_3', frames: this.anims.generateFrameNumbers('cat_wake', [0]), frameRate: 0, repeat: 0 });
				...
				if (!isWakingUp) {
					...
					catSprite.once('animationcomplete', (anim) => {
						...
						if (anim.key === 'cat_wake_2') {
							...
							catSprite.setTexture('cat_wake_2',[3]);  // 여기에 마지막 프레임의 텍스처를 설정
